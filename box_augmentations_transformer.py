...
## the current goal is to perform instance recognition so they are subsistuted by perspective and affine transforms
...
def transforms_preset_5b(min_visibility):
    preset = A.Compose([
        A.Affine(p=0.5),
        A.RandomBrightnessContrast(p=0.2),
        A.RGBShift(r_shift_limit=30, g_shift_limit=30, b_shift_limit=30, p=0.3),
...
def transforms_preset_5c(min_visibility):
    preset = A.Compose([
        A.Perspective(p=0.8),
        A.RandomBrightnessContrast(p=0.2),
    ], bbox_params=A.BboxParams(format='coco', label_fields=['class_labels'],
...
TRANSFORMS_DICT = {
    ...
    'preset_5b': transforms_preset_5,
    'preset_5c': transforms_preset_5,
